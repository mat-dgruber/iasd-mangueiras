# DateTimePickerComponent & Admin Horários Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement a standalone, accessible, zero-dependency `DateTimePickerComponent` in Angular 21 with Tailwind CSS, integrating it into the special notice modal in `admin-horarios.page.ts` and ensuring clean public rendering in `horarios.page.ts`.

**Architecture:** Standalone Angular component implementing `ControlValueAccessor` and Signals `model<string>()` with responsive dropdown popover, date grid (days, months, decade), time selectors (hours, minutes) and ISO value storage with localized pt-BR trigger display.

**Tech Stack:** Angular 21 (Signals + Standalone + ReactiveFormsModule), Tailwind CSS v4, Jasmine/Karma for unit tests.

## Global Constraints

- **Language:** Code/comments/commits in English; UI copy/labels in Brazilian Portuguese (`pt-BR`).
- **Design System:** Use Tailwind CSS utility classes and `advent-*` design tokens (`advent-blue`, `advent-border`, `advent-text`, `advent-muted`).
- **Accessibility:** Touch targets ≥ 44px, `focus-visible:ring-2`, keyboard navigation (`Escape`, `Tab`, `Enter`), ARIA attributes.
- **Zero Third-Party Bloat:** Native Angular & DOM, no heavy datepicker libraries.

---

### Task 1: Create DateTimePickerComponent Unit Tests

**Files:**
- Create: `frontend/src/app/shared/ui/datetime-picker/datetime-picker.component.spec.ts`

**Interfaces:**
- Produces: Test suite validating `ControlValueAccessor`, signals, date navigation, time changes, shortcuts ("Hoje", "Agora", "Limpar"), keyboard interactions.

- [ ] **Step 1: Write the unit tests for DateTimePickerComponent**
...
describe('DateTimePickerComponent', () => {
  ...
  it('should format ISO string to localized display string', () => {
  ...
  it('should update form control value when selecting a day and time', () => {
    ...
    expect(host.control.value).toContain('T19:45:00');
  });
...
- [ ] **Step 2: Run test to verify it fails (component doesn't exist yet)**

Run: `cd frontend && npm test -- --include src/app/shared/ui/datetime-picker/datetime-picker.component.spec.ts --watch=false`
Expected: FAIL with compilation error (cannot find `datetime-picker.component`).

---

### Task 2: Implement DateTimePickerComponent

**Files:**
- Create: `frontend/src/app/shared/ui/datetime-picker/datetime-picker.component.ts`

**Interfaces:**
- Consumes: Angular core signals (`signal`, `computed`, `model`, `input`), `ControlValueAccessor`, `NG_VALUE_ACCESSOR`.
- Produces: `DateTimePickerComponent` standalone with selector `app-ui-datetime-picker`.

- [ ] **Step 1: Implement DateTimePickerComponent**
...
- [ ] **Step 3: Commit component and unit tests**

```bash
git add frontend/src/app/shared/ui/datetime-picker/
git commit -m "feat(ui): add accessible DateTimePickerComponent with date and time support"
```

---

### Task 3: Integrate DateTimePickerComponent into Admin Horários & Update Display
...
**Interfaces:**
- Consumes: `DateTimePickerComponent` from `../../../shared/ui/datetime-picker/datetime-picker.component`.
- Produces: Polished special notices modal with DateTimePickers for `data_evento` and `expira_em`, plus localized badges.

- [ ] **Step 1: Update `admin-horarios.page.ts` to import and use `DateTimePickerComponent`**

In `admin-horarios.page.ts`:
- Import `DateTimePickerComponent`.
- In modal template, replace the raw `data_evento` and `expira_em` inputs with `<app-ui-datetime-picker formControlName="data_evento" ... />` and `<app-ui-datetime-picker formControlName="expira_em" ... />`.
- Add helper method `formatDateTimeDisplay(val?: string): string` to format ISO dates cleanly in the admin list of notices.

- [ ] **Step 2: Update `horarios.page.ts` for public formatted display**

In `horarios.page.ts`:
- Add helper method `formatAvisoDate(val?: string): string` to display `31/12/2026 às 20:00` when ISO string is detected, preserving legacy free-text strings as-is.

- [ ] **Step 3: Run full frontend tests and type checks**

Run: `cd frontend && npm test -- --watch=false`
Run: `cd frontend && npx tsc --noEmit`
Expected: All tests PASS, TypeScript compilation 100% clean.

- [ ] **Step 4: Commit integration changes**

```bash
git add frontend/src/app/features/admin/horarios/admin-horarios.page.ts frontend/src/app/features/horarios/horarios.page.ts
git commit -m "feat(admin): integrate DateTimePicker in admin horarios special notices modal"
```

---

### Task 4: Final Verification & Subagent Gate

**Files:**
- Verify all touched files across the project.

- [ ] **Step 1: Execute production build**

Run: `cd frontend && npm run build`
Expected: Build successfully created without warnings or bundle errors.

- [ ] **Step 2: Run verification subagent**

Spawn `verification` subagent to validate all acceptance criteria before completion.
