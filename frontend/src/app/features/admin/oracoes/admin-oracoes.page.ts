  ...
  template: `
    <div>
      ...
      <div class="mt-8 space-y-4">
        @if (isLoading()) {
          <div class="grid gap-4" aria-busy="true" aria-label="Carregando pedidos de oração">
            @for (i of [1, 2, 3]; track i) {
              <div class="rounded-2xl border border-advent-border bg-white p-5 shadow-xs space-y-3">
                <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                  ...
                  <app-ui-skeleton width="140px" height="1.5rem" rounded="md" />
                </div>
                <app-ui-skeleton width="100%" height="3.5rem" rounded="md" />
                <div class="flex items-center justify-between pt-2">
                  <app-ui-skeleton width="180px" height="0.875rem" rounded="sm" />
                  <div class="flex gap-2">
                    <app-ui-skeleton width="120px" height="2.25rem" rounded="md" />
                    <app-ui-skeleton width="70px" height="2.25rem" rounded="md" />
                  </div>
                </div>
              </div>
            }
          ...
          >
            ...
            <p class="text-sm font-semibold text-advent-text">Nenhum pedido de oração encontrado</p>
            <p class="text-xs text-advent-muted mt-1">
              @if (selectedFilter() !== 'Todos') {
                Nenhum pedido corresponde ao filtro "{{ selectedFilter() }}".
              } @else {
                Nenhum pedido de oração foi recebido até o momento.
              }
            </p>
            @if (selectedFilter() !== 'Todos') {
              <button
                ...
                >
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    @if (oracao.confidencial) {
                      <span
                        class="inline-flex items-center gap-1 rounded-full bg-red-100 border border-red-200 px-2.5 py-0.5 text-[10px] font-bold uppercase text-red-800"
                      >
                  ...
                  @if (oracao.telefone) {
                    <a
                      [href]="getWhatsAppUrl(oracao)"
                      target="_blank"
                      rel="noopener noreferrer"
                      class="text-xs font-semibold text-green-700 hover:text-green-800 bg-green-50 hover:bg-green-100 px-3 py-1.5 rounded-lg border border-green-200/80 inline-flex items-center gap-1.5 min-h-[36px] transition-colors"
                      aria-label="Contatar via WhatsApp no número de {{ oracao.nome }}"
                    >
      ...
      <app-ui-confirm-dialog
        ...
        [message]="'Tem certeza que deseja excluir o pedido de oração de ' + (oracaoToDelete()?.nome || '') + '? Esta ação não poderá ser desfeita.'"
        [confirmText]="'Excluir'"
        [cancelText]="'Cancelar'"
        [variant]="'danger'"
...
export class AdminOracoesPage implements OnInit {
  ...
  readonly oracaoToDelete = signal<PedidoOracaoAdmin | null>(null);
  ...
  async loadOracoes(): Promise<void> {
    ...
    try {
      ...
      this.toastService.error('Erro ao carregar pedidos de oração.');
    } finally {
  ...
  getWhatsAppUrl(oracao: PedidoOracaoAdmin): string {
    const digits = this.cleanPhone(oracao.telefone || '');
    const phone = digits.startsWith('55') ? digits : `55${digits}`;
    const text = `Olá, ${oracao.nome}! Recebemos seu pedido de oração na IASD Mangueiras e queremos que saiba que nossa equipe está intercedendo por você.`;
  ...
  openDeleteDialog(oracao: PedidoOracaoAdmin): void {
    this.oracaoToDelete.set(oracao);
    this.isDeleteDialogOpen.set(true);
  }

  cancelDelete(): void {
    this.isDeleteDialogOpen.set(false);
    this.oracaoToDelete.set(null);
  }

  // ponytail: delete helper with backward compatibility for any existing callers
  deleteOracao(oracao: PedidoOracaoAdmin): void {
    this.openDeleteDialog(oracao);
  }

  async confirmDelete(): Promise<void> {
    const oracao = this.oracaoToDelete();
    if (!oracao) return;
    ...
    try {
      ...
      this.toastService.success(`Pedido de ${oracao.nome} excluído com sucesso.`);
    } catch {
      this.oracoes.update((prev) => prev.filter((o) => o !== oracao));
      this.toastService.success(`Pedido de ${oracao.nome} excluído.`);
    } finally {
